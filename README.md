...
There is also an automated UI smoke test that drives the real window through 112 checks (documents, tools, effects, undo, save/reopen, themes, menus, dialogs, startup chrome, exact-corner canvas panning, layer-targeted paste, stable resize/rotation transforms, drag/drop activation, active-layer clipboard behavior, Enter-to-deselect, and visible multi-edge docking):
