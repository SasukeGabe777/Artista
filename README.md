...
- **Canvas**: checkerboard transparency, cursor-centered wheel zoom (3%–6400%), fit-to-window / actual size, smooth panning (Space+drag, middle-drag, Pan tool), scrollbars, pixel grid at high zoom, configurable red Sprite Grid with cell size/origin/spacing, F1 visibility toggle, frame-cell snapping/highlighting for Rectangle Select, optional rulers, animated marching-ants selection outline, live tool previews.
...
- **Sprite animation**: turn a selected/parked sprite strip, two or more pasteboard pieces, or separately selected sprite-sheet regions into an Aseprite-style Sprite Canvas. Alpha-first sprite analysis can losslessly center misaligned sprites in their majority-overlap cells, align only the grid origin across many detected sprites, or infer cell size/origin/spacing with Detect Sprite Grid; every automatic operation has a visual preview, and pixel alignment is one atomic undo step. The Sprite Canvas includes frame thumbnails, playback/scrubbing, loop and ping-pong modes, per-frame timing, speed and integer-zoom controls, frame reordering, animated GIF import, and animated GIF export.
...
- `docs/KEYBOARD_SHORTCUTS.md` — full shortcut list (also in-app: Help → Keyboard Shortcuts)
